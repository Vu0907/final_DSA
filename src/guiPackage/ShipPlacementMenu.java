...
public class ShipPlacementMenu {
	...
	public ShipPlacementMenu(JFrame f, boolean sEffects, boolean sTrack, String gameMode, Clip clip,
			int numberOfPlayers) {
		frame = f;
		...
		helpButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				JOptionPane.showMessageDialog(new JFrame(),
						"Drag the ships with the mouse to place them on the board.\n"
								+ "To change the direction of a ship from horizontal to vertical(and the opposite) scroll the mouse.\n"
								+ "Between the ships there must be a gap of at least one rectangle.",
						"Help",
		...
		nextButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				for (int i = 0; i < 6; i++) {
					Connect.setCoordinates(i, getMyRectColumn(myRect[i]), getMyRectRow(myRect[i]),
							isMyRectHorizontal(myRect[i]));
				}
				...
				if (Connect.getPlacementStatus()) {
					...
					JOptionPane.showMessageDialog(new JFrame(),
							"Put all the ships in the table, and make sure\n that there "
									+ "is at least one space between them.",
							"Error",
